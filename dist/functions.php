...
function insert_fb_in_head() {
    ...
    if ( !is_singular()) //if it is not a post or a page
        ...
        echo '<meta property="og:site_name" content="blog.musikalienhandel.de"/>';
    if(!has_post_thumbnail( $post->ID )) { //the post does not have featured image, use a default image
        $default_image="https://blog.musikalienhandel.de/image.jpg"; //replace this with a default image on your server or an image in your media library
...
function preprocess_new_comment($commentdata) {
	if(isset($_POST['urlopulrlo'])) {
		if(strlen($_POST['urlopulrlo']) > 2){
			echo "_error_"; die;
		}
	}
	return $commentdata;
...
function kubrick_add_theme_page() {
	...
	add_theme_page('Header konfigurieren', 'Headergrafik und -Farben', 'edit_themes', basename(__FILE__), 'kubrick_theme_page');
}
...
// <![CDATA[
	...
	function PopupWindow_populate(contents) {
		contents += '<br /><p style="text-align:center;margin-top:0px;"><input type="button" value="Farbwhler schlieen" onclick="cp.hidePopup(\'prettyplease\')"></input></p>';
...
function kubrick_theme_page() {
	if ( $_REQUEST['saved'] ) echo '<div id="message" class="updated fade"><p><strong>&auml;nderungen gespeichert.</strong></p></div>';
?>
<div class='wrap'>
	<div id="kubrick-header">
		<h2>Header und Farben</h2>
		<div id="headwrap">
			<div id="header">
				<div id="headerimg">
		...
		<div id="nonJsForm">
			<form method="post" action="">
				...
				<label for="njfontcolor">Schriftfarbe:</label><input type="text" name="njfontcolor" id="njfontcolor" value="<?php echo kubrick_header_color(); ?>" /> CSS-Farben (<code>red</code> oder <code>#FF0000</code> oder <code>rgb(255, 0, 0)</code>)<br />
				<label for="njuppercolor">Farbverlauf oben:</label><input type="text" name="njuppercolor" id="njuppercolor" value="#<?php echo kubrick_upper_color(); ?>" /> Nur HEX-Zahlen (<code>#FF0000</code> oder <code>#F00</code>)<br />
				<label for="njlowercolor">Farbverlauf unten:</label><input type="text" name="njlowercolor" id="njlowercolor" value="#<?php echo kubrick_lower_color(); ?>" /> Nur HEX-Zahlen (<code>#FF0000</code> oder <code>#F00</code>)<br />
				<input type="hidden" name="hi" id="hi" value="<?php echo kubrick_header_image(); ?>" />
				<input type="submit" name="toggledisplay" id="toggledisplay" value="Schrift ausblenden" />
				<input type="submit" name="defaults" value="Voreinstellung" />
				<input type="submit" class="defbutton" name="submitform" value="&nbsp;&nbsp;Speichern&nbsp;&nbsp;" />
		...
		<div id="jsForm">
			<form style="display:inline;" method="post" name="hicolor" id="hicolor" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
				<input type="button" onclick="tgt=document.getElementById('fontcolor');colorSelect(tgt,'pick1');return false;" name="pick1" id="pick1" value="Schriftfarbe"></input>
				<input type="button" onclick="tgt=document.getElementById('uppercolor');colorSelect(tgt,'pick2');return false;" name="pick2" id="pick2" value="Farbverlauf oben"></input>
				<input type="button" onclick="tgt=document.getElementById('lowercolor');colorSelect(tgt,'pick3');return false;" name="pick3" id="pick3" value="Farbverlauf unten"></input>
				<input type="button" name="revert" value="Zur&uuml;cksetzen" onclick="kRevert()" />
				<input type="button" value="Erweitert" onclick="toggleAdvanced()" />
			...
			<div id="advanced">
				<form id="jsAdvanced" style="display:none;" action="">
					<label for="advfontcolor">Schriftfarbe (CSS): </label><input type="text" id="advfontcolor" onchange="advUpdate(this.value, 'fontcolor')" value="<?php echo kubrick_header_color(); ?>" /><br />
					<label for="advuppercolor">Farbverlauf oben (HEX): </label><input type="text" id="advuppercolor" onchange="advUpdate(this.value, 'uppercolor')" value="#<?php echo kubrick_upper_color(); ?>" /><br />
					<label for="advlowercolor">Farberlauf unten (HEX): </label><input type="text" id="advlowercolor" onchange="advUpdate(this.value, 'lowercolor')" value="#<?php echo kubrick_lower_color(); ?>" /><br />
					<input type="button" name="default" value="Standardfarben w&auml;hlen" onclick="kDefaults()" /><br />
					<input type="button" onclick="toggleDisplay();return false;" name="pick" id="pick" value="Schrift ausblenden"></input><br />
				</form>
			</div>
		</div>
	</div>
